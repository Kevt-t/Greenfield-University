...
app.get("/student-dashboard", authToken, (req, res) => {
    if (req.user.role !== "Student") {
        return res.redirect("/login");
    }
    res.render("student-dashboard", { 
        email: req.user.email, 
        enrollment: req.user.enrollment // Ensure enrollment is passed
    });
});

app.get("/instructor-dashboard", authToken, (req, res) => {
    if (req.user.role !== "Instructor") {
        return res.redirect("/login");
    }
    res.render("instructor-dashboard", { 
        email: req.user.email, 
        enrollment: req.user.enrollment // Pass enrollment here too
    });
});
